(function (app) {
    ...
    function photoUploadController($scope, myService, $http, $uibModal, $log, $document, $window, $base64, $stateParams, swalAlert) {
        ...
        $ctrl.uploadFile = function () {
            ...
            var fd = new FormData();
            fd.append('file', file);
            var queryString = '?type=&candidateID=' + $scope.candidateId;
            $http.post('api/UploadFileDataPelengkap/UploadFile' + queryString, fd, {
    ...
    function recruitmentFormController($scope, myService, $http, $uibModal, $log, $document, $window, $stateParams, $state, authService, $compile, $timeout, swalAlert, $q) {
        ...
        function getCandidate(candidateid) {
            ...
            getDataCandidate(candidateid).then(function (response) {
                ...
                if ($scope.candidate.Status == 'DRAFT') {
                    //$scope.candidate.LocationCode = Upper($scope.candidate.LocationCode);
                    $scope.candidate.LocationCode = ($scope.candidate.LocationCode);
                    if ($scope.candidate.SpouseBirthDate) {
                        ...
                        if ($scope.candidate && $scope.candidate.AajiExam && typeof ($scope.candidate.AajiExam.AajiExamId) != 'undefined' && $scope.candidate.AajiExam.AajiExamId) {
                            var s = $scope.candidate.AajiExam.AajiExamDate;
                            if (s) {
                                $scope.candidate.AajiExam.AajiExamDate = s.substring(0, s.indexOf('T'));
                            }
                        }
                    } else {
                        //console.log('tidak ada spouse birthdate');
                    }
                }

                loadPTKP();

                $scope.listEducation = $scope.candidate.Educations;
                $scope.listExperience = $scope.candidate.Experiences;
                ...
                if ($scope.candidate.Status == 'DRAFT') {
                    $scope.candidate.LocationCode = $scope.candidate.LocationCode.toUpperCase();
                }
